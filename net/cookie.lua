...
function M.parse_set_cookie(c, quoted, cookie_table)
    c = c .. ";$last=last;"
...
function M.split_set_cookie(s, cookie_table)
    ...
    while 1 do
        ...
        M.parse_set_cookie(cookie, quoted, cookie_table)
...
local _empty = {}
function M.build_cookies(cookies)
    s = ""
...
---@return string|nil
function M.value(cookie_jar, host, key)
  for _,v in pairs(cookie_jar[host]) do
    if v.name == key then
      return v.value
    end
  end
